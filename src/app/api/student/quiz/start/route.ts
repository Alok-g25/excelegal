    ...
    try {
        ...
        if (studentProfile) {
            ...
            if (attemptedQuestions?.length < quiz?.no_of_questions) {
                ...
                attemptedModel.course = quiz?.course
                ...
                if (attemptedQuestions.length === 0) {
                    attemptedModel.time.start = Date.now()
                }
                if ((quiz?.no_of_questions - attemptedQuestions.length) === 1) {
                    attemptedModel.time.end = Date.now()
                }
                ...
                if (updatedAttemptedQuestions)
                    if (updatedAttemptedQuestions?.length < updatedquiz?.no_of_questions) {
                        ...
                        return NextResponse.json({ success: true, code: 200, data: { totalQuestion: updatedquiz?.no_of_questions, weightage: updatedquiz?.weightage, totalMarks: maxMarks, finalResult: totalMarks, percentage: `${percentage.toFixed(2)}%`, time: updatedAttemptedQuestions.find(a => a?.timeTaken)?.timeTaken } });
                    }
            } else {
                ...
                const percentage = (totalMarks / maxMarks) * 100;
                console.log(attemptedQuestions.find(a => a?.time?.start)?.time?.start, '-------- ok')
                return NextResponse.json({ success: true, code: 200, data: { totalQuestion: quiz?.no_of_questions, weightage: quiz?.weightage, totalMarks: maxMarks, finalResult: totalMarks, percentage: `${percentage.toFixed(2)}%`, time: { start: attemptedQuestions.find(a => a?.time?.start)?.time?.start, end: attemptedQuestions.find(a => a?.time?.end)?.time?.end } } });
            }
        } else {
